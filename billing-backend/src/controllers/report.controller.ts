...
export class ReportController {
  ...
  async getSalesReport(req: BusinessRequest, res: Response): Promise<Response | unknown> {
    try {
  ...
  async getTopSellingProducts(req: BusinessRequest, res: Response): Promise<Response | unknown> {
      try {
      const businessId = req.business?.id;
      if (!businessId) {
        return res.status(401).json({ success: false, message: 'Unauthorized' });
  ...
  async getInventoryValuation(req: BusinessRequest, res: Response): Promise<Response | unknown> {
    try {
        const businessId = req.business?.id;
        const user = req.user;
        ...
        return res.status(500).json({ success: false, message: 'Failed to fetch inventory valuation' });
    }
  }

  async getLowStockAlerts(req: BusinessRequest, res: Response): Promise<Response | unknown> {
      try {
          const businessId = req.business?.id;
          if (!businessId) {
            return res.status(401).json({ success: false, message: 'Unauthorized' });
  ...
  async getProfitLoss(req: BusinessRequest, res: Response): Promise<Response | unknown> {
      try {
        const businessId = req.business?.id;
        const user = req.user;
        ...
        return res.status(500).json({ success: false, message: 'Failed to fetch profit loss report' });
      }
  }

  async getTaxReport(req: BusinessRequest, res: Response): Promise<Response | unknown> {
    try {
      const businessId = req.business?.id;
      const user = req.user;
      ...
      return res.status(500).json({ success: false, message: 'Failed to fetch tax report' });
    }
  }
}
